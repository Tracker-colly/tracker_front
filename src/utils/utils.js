...
export const patternPwCker1 = /^(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!@#$%^&*?_]).{8,20}$/;
export const patternPwCker2 = /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*?_]).{8,20}$/;
...
export const dateMinformat2 = (value, isWeek = true) => {
    let week = krWeekList[moment(value).day()];
    return moment(value).format(`YYYY.MM.DD${isWeek ? " (" + week + ")" : ""}`);
};
...
export const yearList = (start) => {
    let list = []
    let startYear = start ? start : 1971;
...
export const elapsedTime = (date) => {
    ...
    return dateMinformat2(start, true);
...
export function getfileSize(x) {
    var s = ['kB', 'MB', 'GB', 'TB', 'PB'];
    var e = Math.floor(Math.log(x) / Math.log(1024));
    return (x / Math.pow(1024, e)).toFixed(2) + " " + s[e];
};

export function defaultProfile(e) {
    e.currentTarget.src = images.default_profile
}

export function wait(num = 1000) {
